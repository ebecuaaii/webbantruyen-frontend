...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const apiRequest = async (path, options = {}) => {
    ...
    if (!response.ok) {
      ...
      const message =
        payload?.message || payload?.data?.message || "Request failed";
    ...
    return products
      .map((product) => {
        ...
        const imageUrl = escapeHtml(
          product.imageUrl ||
            "https://placehold.co/200x250/fff/333?text=Product",
        );
      ...
      if (!item.querySelector("a")) {
        ...
        if (link) {
          ...
          item.onclick = () => {
            ...
            if (type === "favorite")
              window.location.href = `${base}wishlist.html`;
          };
        }
      }
    });
  ...
  const isHomePage = (() => {
    ...
    return (
      homePath.endsWith("/index.html") || homePath === "/" || homePath === ""
    );
  })();
  ...
  const renderHomeFeaturedProducts = (products) => {
    ...
    if (!container) return;
    container.innerHTML = buildProductCardsHtml(
      products,
  ...
  const renderHomeMangaProducts = (products) => {
    ...
    container.innerHTML = buildProductCardsHtml(
      products,
      "Chưa có sản phẩm thuộc danh mục Manga",
    );
  };
  ...
  const renderHomeMerchProducts = (products) => {
    ...
    container.innerHTML = buildProductCardsHtml(
      products,
      "Chưa có sản phẩm thuộc danh mục Merch",
    );
  };
    ...
    container.innerHTML = newsItems
      .map((item) => {
        ...
        const thumbnail = escapeHtml(
          item.thumbnail ||
            "https://placehold.co/300x225/ffffff/9f2425?text=News",
        );
  ...
  const loadHomeData = async () => {
    ...
    try {
      const data = await apiRequest(
        "/api/home?categoryLimit=8&productLimit=8&newsLimit=6",
      );
      ...
      const mangaCategory = categories.find(
        (item) => (item?.name || "").trim().toLowerCase() === "manga",
      );
      if (mangaCategory?.id) {
        const mangaProducts = await apiRequest(
          `/api/products?categoryId=${encodeURIComponent(mangaCategory.id)}`,
        );
        renderHomeMangaProducts((mangaProducts || []).slice(0, 8));

        const viewMoreBtn = document.getElementById("homeMangaViewMore");
        if (viewMoreBtn) {
          viewMoreBtn.setAttribute(
            "href",
            `pages/products.html?categoryId=${encodeURIComponent(mangaCategory.id)}`,
          );
        }
      } else {
      ...
      const merchCategory = categories.find(
        (item) => (item?.name || "").trim().toLowerCase() === "merch",
      );
      if (merchCategory?.id) {
        const merchProducts = await apiRequest(
          `/api/products?categoryId=${encodeURIComponent(merchCategory.id)}`,
        );
        renderHomeMerchProducts((merchProducts || []).slice(0, 8));

        const merchViewMoreBtn = document.getElementById("homeMerchViewMore");
        if (merchViewMoreBtn) {
          merchViewMoreBtn.setAttribute(
            "href",
            `pages/products.html?categoryId=${encodeURIComponent(merchCategory.id)}`,
          );
        }
      } else {
  ...
  const loadNewsDetailPage = async () => {
    ...
    const breadcrumbTitleEl = document.getElementById(
      "newsDetailBreadcrumbTitle",
    );
    ...
    try {
      ...
      if (slug) {
        ...
        if (firstItem?.slug) {
          detail = await apiRequest(
            `/api/news/slug/${encodeURIComponent(firstItem.slug)}`,
          );
        }
      }
      ...
      if (publishedAtEl)
        publishedAtEl.textContent = formatNewsDate(detail.published_at, true);
      if (thumbnailEl) {
        thumbnailEl.src =
          detail.thumbnail || "https://placehold.co/800x450/fff/333?text=News";
        thumbnailEl.alt = title;
      }
      titleEl.innerHTML = escapeHtml(title).replace(/\n/g, "<br />");

      const content =
        detail.content || summary || "Nội dung đang được cập nhật.";
      const isHtmlContent = /<[^>]+>/.test(content);
      contentEl.innerHTML = isHtmlContent
        ? content
          ...
          relatedItems
            .map((item) => {
              ...
              const image = escapeHtml(
                item.thumbnail ||
                  "https://placehold.co/150x110/ffffff/9f2425?text=News",
              );
              const detailUrl = item.slug
                ? `news-detail.html?slug=${encodeURIComponent(item.slug)}`
                : `news-detail.html?id=${encodeURIComponent(item.id || "")}`;
      ...
      if (breadcrumbTitleEl)
        breadcrumbTitleEl.textContent = "Không tìm thấy bài viết";
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (event) => {
      ...
      const usernameOrEmail = loginForm
        .querySelector('[name="usernameOrEmail"]')
        .value.trim();
  ...
  if (adminLoginForm) {
    adminLoginForm.addEventListener("submit", async (event) => {
      event.preventDefault();
      const usernameOrEmail = adminLoginForm
        .querySelector('[name="usernameOrEmail"]')
        .value.trim();
    ...
    apiRequest("/api/users/me")
      .then((user) => {
        if (profileInfo.fullName)
          profileInfo.fullName.textContent = user.fullName || "-";
        if (profileInfo.email)
          profileInfo.email.textContent = user.email || "-";
        if (profileInfo.phone)
          profileInfo.phone.textContent = user.phone || "-";
        const displayName = user.fullName || user.username || "";
        if (profileInfo.displayName)
          profileInfo.displayName.textContent = displayName;
        if (profileInfo.headerName)
          profileInfo.headerName.textContent = displayName;
        ...
        if (profileForm) {
          profileForm.querySelector('[name="fullName"]').value =
            user.fullName || "";
          profileForm.querySelector('[name="phone"]').value = user.phone || "";
          profileForm.querySelector('[name="avatar"]').value =
            user.avatar || "";
        }
      })
      .catch(() => {
        if (profileForm)
          showMessage(profileForm, "Không thể tải thông tin user");
      });
  }
    ...
    profileForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (profileInfo.phone)
          profileInfo.phone.textContent = user.phone || "-";
        if (profileInfo.displayName) {
          profileInfo.displayName.textContent =
  ...
  if (adminUserTable) {
    ...
    adminUserTable.addEventListener("click", async (event) => {
      ...
      if (action === "edit") {
        ...
        adminUserForm.querySelector('[name="username"]').value =
          user.username || "";
        adminUserForm.querySelector('[name="email"]').value = user.email || "";
        adminUserForm.querySelector('[name="password"]').value = "";
        adminUserForm.querySelector('[name="fullName"]').value =
          user.fullName || "";
        adminUserForm.querySelector('[name="phone"]').value = user.phone || "";
        adminUserForm.querySelector('[name="avatar"]').value =
          user.avatar || "";
        adminUserForm.querySelector('[name="role"]').value =
          user.role || "USER";
  ...
  if (
    ...
    fetchProducts().then((data) => {
      ...
      if (grids[0])
        renderProducts(
          data.slice(0, 4),
          ".product-section:nth-of-type(1) .product-grid",
        );
      if (grids[1])
        renderProducts(
          data.slice(4, 8),
          ".product-section:nth-of-type(2) .product-grid",
        );
    });
  }
